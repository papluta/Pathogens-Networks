...
load('Data/Results/250304_models_morisita.RData')
...
dw = data.both  %>% pivot_longer(cols = dwvb:sbv, names_to = 'virus', values_to = 'presence') %>% group_by(Group, virus) %>%
  ...
  ylim(0,1)+
  labs(x = NULL, y = 'Prevalence', fill = NULL, title = expression(bold('c')))+
...
bq = data.both  %>% pivot_longer(cols = dwvb:sbv, names_to = 'virus', values_to = 'presence') %>% group_by(Group, virus) %>%
  ...
  theme_bw(base_size = 16)+
  ylim(0,1)+
  labs(x = NULL, y = 'Prevalence', fill = NULL, title = expression(bold('f')))+
...
prev.sep = plot_grid(dw + theme(legend.position = 'none'), bq, nrow = 2)
...
dwvb.top.r0 = r0.plot %>% filter(Virus == 'dwvb') %>% filter(Species %in% top4[top4$Virus == 'dwvb',]$Species) %>%
  ...
  theme(legend.position = 'none', axis.text.y = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  labs(x = NULL, y = expression(paste('DWV-B ', R[0][","][i])), title = expression(bold('a')))+
  ylim(0,3)
...
bqcv.top.r0 = r0.plot %>% filter(Virus == 'bqcv') %>% filter(Species %in% top4[top4$Virus == 'bqcv',]$Species) %>%
  ...
  theme(legend.position = 'none', axis.text.y = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  labs(x = NULL, y = expression(paste('BQCV ', R[0][","][i])), title = expression(bold('b')))+
...
abpv.top.r0 = r0.plot %>% filter(Virus == 'abpv') %>% filter(Species %in% top4[top4$Virus == 'abpv',]$Species) %>%
  ...
  theme(legend.position = 'none', axis.text.y = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  labs(x = NULL, y = expression(paste('ABPV ', R[0][","][i])), title = expression(bold('c')))+
  ylim(0,6)
...
sim.logodds = sim %>% mutate(Prev.x.odds = log(Prev.x/(1 - Prev.x)), Prev.y.odds = log(Prev.y/(1 - Prev.y))) %>%
  mutate(Fold.odds = Prev.x.odds / Prev.y.odds)
...
mor.a = ggplot(posterior_preds, aes(x = sum.ab.fl.s, y = .epred)) +
  stat_lineribbon(.width = c(0.95), alpha = 0.5, aes(fill = as.factor(FL_rich)), show.legend = F) +
  labs(x = "Total bee density", y = "Resource overlap with\nhoneybee", title = expression(bold('a')), col = 'Flower richness')+
  geom_jitter(data = mod$data, aes(sum.ab.fl.s, Morisita.z, col = cut(FL_rich, 2)), shape = 19, height = 0.025, alpha = 0.5)+ 
  #scale_y_continuous(limits = c(-5, 2.5))+
  theme_bw(base_size = 16)+
  scale_color_manual(values = c('#ddcd4b','#4b5bdd'), labels = c('2 - 9', '< 9'))+
  scale_fill_manual(values = c('#ddcd4b','#4b5bdd'), labels = c('2 - 9', '< 9'))+
...
mor.b = ggplot(posterior_preds, aes(x = sum.ab.fl.s, y = .epred)) +
  stat_lineribbon(.width = c(0.95), alpha = 0.5, aes(fill = as.factor(FL_rich)), show.legend = F) +
  labs(x = "Total bee density", y = "Resource overlap with\nred-tailed bumblebee", title = expression(bold('b')), col = 'Flower richness')+
  geom_jitter(data = mod$data, aes(sum.ab.fl.s, Morisita.bl.z, col = cut(FL_rich, 2)), shape = 19, height = 0.025, alpha = 0.5)+ 
...
con.a = ggplot(posterior_preds2, aes(x = r, y = .epred)) +
  ...
  labs(x = "Total bee density (ln)", y = "Network connectance", title = expression(bold('a')))+
...
con.b = ggplot(posterior_preds, aes(x = FL_rich, y = .epred)) +
  stat_lineribbon(.width = c(0.95), fill = '#4b5bdd', show.legend = F) +
  labs(x = "Flower richness", y = "Network connectance", title = expression(bold('b')))+
...
wb.cs <- lapply(WB_models_Morisita, custom_summary) %>% bind_rows(.id = 'id') 
...
wb.cs2 <- wb.cs %>% mutate(term = ifelse(grepl('hu_', Predictor), 'Probability of presence', 'Viral load')) %>% 
  ...
  mutate(Predictor = recode_factor(as.factor(Predictor), 'dwvb.f.s' = 'Transmission potential', 'bqcv.f.s' = 'Transmission potential', 'abpv.bl.f.s' = 'Transmission potential', 
                                   'Bee_rich' = 'Bee richness', 'Year2022' = 'Year 2022', 'Morisita.z.s' = 'HB niche overlap', 'Morisita.bl.z.s' = 'BL niche overlap',
                                   'SpeciesBombuspascuorum' = 'B. pascuorum', 'SpeciesBombusterrestris' = 'B. terrestris', "Connectance.s" = "Connectance", "sum.ab.fl.s" = "Total bee density"),
...
exp_dwvb_w = ggplot(posterior_preds2, aes(x = r, y = 1-hu)) +
  ...
  theme(legend.position = "none", panel.grid = element_blank(), axis.text = element_text(color = 'black'))+
  add_phylopic(img = img_BT, x = 2.7, y = 0.85, height = 0.2)
...
exp_bqcv_w = ggplot(posterior_preds2, aes(x = r, y = 1-hu)) +
  ...
  scale_y_continuous(breaks = c(0,0.5,1))+
  theme(legend.position = "none", panel.grid = element_blank(), axis.text = element_text(color = 'black'))+
  add_phylopic(img = img_BT, x = 12, y = 0.85, height = 0.2)
...
exp_abpv_w = ggplot(posterior_preds2, aes(x = r, y = 1-hu)) +
  ...
  theme_bw(base_size = 14)+
  theme(legend.position = "none", panel.grid = element_blank(), axis.text = element_text(color = 'black'))+
  add_phylopic(img = img_BT, x = 2, y = 0.85, height = 0.2)
...
nich_bqcv_b = ggplot(posterior_preds, aes(x = Morisita.z.s, y = 1-hu)) +
  ...
  geom_jitter(data = mod$data %>% mutate(BQCV.abs = ifelse(BQCV.abs > 0, 1, 0)), aes(Morisita.z.s, BQCV.abs), 
  ...
  theme(legend.position = "none", panel.grid = element_blank(), axis.text = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  add_phylopic(img = img_BT, y = 0.85, x = -3.8, height = 0.2)

posterior_preds <- epred_draws(mod, newdata = new_dat, dpar = 'mu', re_formula = NA)

nich_bqcv_b_mu = ggplot(posterior_preds, aes(x = Morisita.z.s, y = mu)) +
  stat_lineribbon(.width = c(0.95), fill = '#6a7f54') +
  labs(x = expression(paste("Resource overlap with ", italic("A. mellifera"))), y = "BQCV load (ln)", 
       title = expression(bold('b')))+
  annotate("text", label = "pd = 0.99", x=-1.6,  y=16, hjust=0)+
  geom_jitter(data = mod$data[mod$data$BQCV.abs >0,], aes(Morisita.z.s, log(BQCV.abs)), shape = 21, height = 0.025, 
              alpha = 0.4)+
  theme_bw(base_size = 14)+
  theme(legend.position = "none", panel.grid = element_blank(), axis.text = element_text(color = 'black'),
        plot.caption = element_text(hjust = 0),
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  add_phylopic(img = img_BT, y = 14, x = -3.6, height = 3)
...
nich_bqcv_w = ggplot(posterior_preds, aes(x = Morisita.z.s, y = mu)) +
  ...
  theme(legend.position = "none", panel.grid = element_blank(), axis.text = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  add_phylopic(img = img_LS, y = 11, x = -1.7, height = 2)
...
nich_abpv_w = ggplot(posterior_preds, aes(x = Morisita.bl.z.s, y = mu)) +
  ...
  geom_jitter(data = mod$data[mod$data$ABPV.abs >0,], aes(Morisita.bl.z.s, log(ABPV.abs)), shape = 21, height = 0.025, 
              alpha = 0.4)+
  theme_bw(base_size = 14)+
  theme(legend.position = "none", panel.grid = element_blank(), axis.text = element_text(color = 'black'),
        plot.caption = element_text(hjust = 0),
...
niche_pred = plot_grid(nich_bqcv_b, nich_bqcv_b_mu, nich_bqcv_w, nich_abpv_w, nrow = 2, align = 'hv')
...
con.a = ggplot(posterior_preds2, aes(x = r, y = 1-hu)) +
  ...
  theme(legend.position = "bottom", panel.grid = element_blank(), axis.text = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  add_phylopic(img = img_BT, x = 0.21, y = 0.85, height = 0.2)
...
con.b = ggplot(posterior_preds2, aes(x = r, y = 1-hu)) +
  ...
  theme(legend.position = "bottom", panel.grid = element_blank(), axis.text = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  add_phylopic(img = img_LS, x = 0.21, y = 0.85, height = 0.2)
...
con.c = ggplot(posterior_preds2, aes(x = r, y = 1-hu)) +
  stat_lineribbon(.width = c(0.95), fill = '#547f7f') +
  labs(x = "Connectance", y = "BQCV prevalence", title = expression(bold('d')))+
...
con.d = ggplot(posterior_preds2, aes(x = r, y = log(.epred))) +
  stat_lineribbon(.width = c(0.95), fill = '#547f7f') +
  labs(x = "Connectance", y = "BQCV load (ln)", title = expression(bold('e')))+
  ...
  theme(legend.position = "bottom", panel.grid = element_blank(), axis.text = element_text(color = 'black'),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")+
  add_phylopic(img = img_BT, x = 0.21, y = 14, height = 2.5)
...
ggsave('Data/fig/pred_niche.pdf', width = 8, height = 8)
ggsave('Data/fig/pred_niche.png', width = 8, height = 8, bg = 'transparent')

## FIGURE 4
con.grid
prev.sep

plot_grid(con.grid, prev.sep, nrow = 1, rel_widths = c(3/4, 1.2/4), align = 'hv')

ggsave('Data/fig/pred_conn_prev.pdf', width = 10, height = 8)
ggsave('Data/fig/pred_conn_prev.png', width = 10, height = 8)

## FIGURE S3

outlier = data.both %>% filter(Species == 'Apis mellifera') %>% mutate(Morisita.bl.z = scale(Morisita.bl.z)[,1]) %>% distinct(Site, Morisita.bl.z)
outlier = data.both %>% filter(Species == 'Bombus lapidarius')  %>% distinct(Site, Morisita.z)

ggplot(outlier, aes(Morisita.bl.z, Site))+
  geom_point(size = 3, shape = 21, aes(fill = ifelse(Morisita.bl.z < -4, 'yes', 'no')))+
  theme_bw(base_size = 16)+
  scale_fill_manual(values = c('white', 'red'))+
  labs(x = expression(atop(paste('Resource overlap between '), paste(italic('A. mellifera '), 'and ', italic('B. lapidarius')))),
       y = 'Site')+
  theme(axis.text = element_text(color = 'black'), panel.grid.major.x = element_blank(), 
        panel.grid.minor.x = element_blank(), legend.position = 'none', axis.text.y = element_text(size = 12))

ggsave(file = 'Data/fig/dotchart_overlap.png', height = 6, width = 6.5)
