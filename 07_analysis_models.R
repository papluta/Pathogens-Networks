...
posterior_correlation <- function(fit = fit, cor = 0.5) {
  ...
  cor_df_clean <- cor_df %>%
    filter(var1 < var2)   # alphabetical comparison 
...
con.m <- brm(Connectance ~ sum_nodes + flower_dens.s + total_bee_dens.s + Year + (1|Site), family = Beta(), # INTERACTION NOT SIGNIFICANT
            ...
            data = data.site)
# con.m.dens <- brm(Connectance ~ sum_nodes + FL.sum.s * total_bee_dens.s + Year  + (1|Site), family = Beta(), # INTERACTION NOT SIGNIFICANT
#              prior = c(prior(normal(0,2), class = 'b'),
#                        prior(exponential(1), class = "sd")), data = data.site)
